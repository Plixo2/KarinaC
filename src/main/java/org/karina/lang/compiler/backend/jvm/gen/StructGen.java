...
import org.karina.lang.compiler.json.JsonBoolean;
...
public class StructGen {
    ...
    public StructGen(BytecodeBackend backend) {
        this.backend = backend;
    }



    public JarOutput addStruct(KTree.KStruct struct) {
    ...
    private static boolean isAnnotation(List<KTree.KAnnotation> annotations, String name) {
        for (var annotation : annotations) {
            if (annotation.name().value().equals(name)) {
                if (annotation.value() instanceof JsonBoolean(Span region, boolean value)) {
                    return value;
                }
                return true;
            }
