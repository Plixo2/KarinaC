...
public final class DidYouMean {
    ...
    public static List<ObjectPath> suggestImportForType(String name, KTree.KPackage root, int limit) {
        var allTypes = new ArrayList<KTree.KTypeItem>();
        findType(name, root, allTypes);

        return allTypes
                .stream()
                .sorted(Comparator.comparingInt(x -> calculate(x.name().value(), name)))
                .limit(limit).map(KTree.KItem::path).toList();
