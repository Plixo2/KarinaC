...
public class AttribExpr {

    KExpr expr;
    AttributionContext ctx;

    protected static AttribExpr attribExpr(@Nullable KType hint, AttributionContext ctx, KExpr expr) {
        if (hint != null) {
            hint = hint.unpack();
        }
        return switch (expr) {
    ...
    public static AttribExpr of(AttributionContext ctx, KExpr expr) {
        var attribExpr = new AttribExpr();
