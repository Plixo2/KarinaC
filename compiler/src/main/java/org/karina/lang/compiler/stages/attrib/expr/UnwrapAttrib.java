...
import org.karina.lang.compiler.utils.symbols.UnwrapSymbol;
...
public class UnwrapAttrib {
    public static AttributionExpr attribUnwrap(
        ...
        var hintInner = getHintType(ctx, hint, expr);
        ...
        if (!(leftType instanceof KType.ClassType classType)) {
            Log.error(ctx, new AttribError.NotAClass(region, leftType));
            throw new Log.KarinaException();
        }

        if (!(ctx.returnType() instanceof KType.ClassType classTypeReturn)) {
            Log.error(ctx, new AttribError.ControlFlow(region, "Return type not compatible with ? operator"));
            throw new Log.KarinaException();
        }

        UnwrapSymbol symbol;
        if (isOptionIndirect(region, ctx, classType)) {
            if (isOption(ctx, region, classTypeReturn)) {
                //none can always be returned
                symbol = new UnwrapSymbol.UnwrapOptional(classType.generics().getFirst());
            } else {
                Log.error(ctx, new AttribError.TypeMismatch(region, classType, classTypeReturn));
                throw new Log.KarinaException();
            }
            ...
            if (isResult(ctx, region, classTypeReturn)) {
                //check if second generic is compatible with return type
                var expectedReturn = classTypeReturn.generics().get(1);

                if (!ctx.checking().canAssign(ctx, region, expectedReturn, errorType, true)) {
                    Log.error(ctx, new AttribError.TypeMismatch(region, expectedReturn, errorType));
                    throw new Log.KarinaException();
                }
            ...
            Log.error(ctx, new AttribError.NotSupportedType(region, leftType));
            throw new Log.KarinaException();
        }
    ...
    private static @Nullable KType getHintType(AttributionContext ctx, @Nullable KType hint, KExpr.Unwrap expr) {
        if (hint instanceof KType.ClassType classType) {
            if (isOption(ctx, expr.region(), classType) || isResult(ctx, expr.region(), classType)) {
    ...
    private static boolean isOption(AttributionContext ctx, Region region, KType.ClassType classType) {
        if (classType.pointer().path().equals(ClassPointer.OPTION_PATH)) {
            if (classType.generics().size() != 1) {
                Log.temp(ctx, region, "Option type must have 1 generics");
                throw new Log.KarinaException();
            }
            return true;
        }
    ...
    private static boolean isResult(AttributionContext ctx, Region region, KType.ClassType classType) {
        if (classType.pointer().path().equals(ClassPointer.RESULT_PATH)) {
            if (classType.generics().size() != 2) {
                Log.temp(ctx, region, "Result type must have 2 generics");
                throw new Log.KarinaException();
            }
            return true;
        }
